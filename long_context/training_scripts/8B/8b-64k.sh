...
#SBATCH --exclude=nid006539,nid007378,nid006931,nid005338,nid006968
...
DATASETS="/capstor/store/cscs/swissai/infra01/users/ctianche/long-ctx-70B/data-mixture/65536_30b"

# 6000 tokens per second per gpu
MBS=1 # Micro batch size
GBS=128 # Global batch size # NOTE(tj.solergibert) Originally this was 2048 BUT we doubled nodecount + GBS. Swicth to rampup batch size feature
SEQ_LEN=65536 # Sequence length
TRAINING_STEPS=3480  # total tokens = 1_075_000 * 8_388_608 = 9_017_753_600_000
CHECKPOINT_STEPS=125 # testing checkpointing
...
EXP_NAME=apertus3-8b-64k-512-nodes-12M
PROJECT_DIR=/capstor/store/cscs/swissai/infra01/users/ctianche/long-ctx-8B-runs/Meg_Runs/$PROJECT_NAME

#########################################
# TODO
LOAD_DIR=/capstor/store/cscs/swissai/infra01/users/ctianche/long-ctx-8B-runs/Meg_Runs/main-long-ctx-runs-8b-v1/apertus3-8b-32k-512-nodes-4M/checkpoints
...
# Remove unfinished checkpoints
LATEST_CONFIRMED=$(cat "$CKPT_DIR/latest_checkpointed_iteration.txt")
LATEST_CONFIRMED=$(printf "%07d" "$LATEST_CONFIRMED")

LATEST_FOLDER=$(find "$CKPT_DIR" -maxdepth 1 -type d -name "iter_*" \
  | sed -n 's/.*iter_\([0-9]*\)$/\1/p' \
  | sort -n | tail -n 1)

# If the highest folder iteration doesn't match the latest confirmed one, remove it
if [ "$LATEST_FOLDER" != "$LATEST_CONFIRMED" ]; then
  echo "[$(date)] Deleting unfinished checkpoint folder: iter_$LATEST_FOLDER"
  rm -rf "$CKPT_DIR/iter_$LATEST_FOLDER"
else
  echo "[$(date)] Checkpoint is consistent: iter_$LATEST_CONFIRMED"
fi
